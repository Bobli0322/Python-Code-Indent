...
#Ver. 2 - Display information about where indentation hint is wrong
...
class indent:#{0
    def __init__(self, fileName):#{1
        ...
        for i in range(self.level):#{2
            ...
            errorDet = 0
            d = 0
            #if errorDet==2 - there is begin but no end
            #if errorDet==-1 - there is end but no begin
    ...
    def checkSemantics(self):#{1
        for i in range(len(self.beginM)):#{2
            if len(self.beginM[i]) != len(self.endM[i]):#{3
                for i in self.error:#{4
                    if i[2] == 2:#{5
                        print('Missing ending marker')
                    #}5
                    elif i[2] == -1:#{5
                        print('Missing beginning marker')
                    #}5
                    print('Error level:' + str(i[0]) + ', ' + 'Error line:' + str(i[1]) + '\n')
                #}4
                return False
            #}3
        #}2
